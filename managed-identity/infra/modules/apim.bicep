...
var responsesPolicy = replace(
  replace(
    ...
    '__RESPONSES_API_CLIENT_ID__',
    responsesApiClientId
  ),
  '__SPA_CLIENT_ID__',
  spaClientId
)
...
resource responsesApi 'Microsoft.ApiManagement/service/apis@2024-05-01' = {
  ...
  properties: {
    displayName: 'Managed Identity Agent Responses'
    path: 'agent'
    protocols: [
      'https'
    ]
...
resource mcpApi 'Microsoft.ApiManagement/service/apis@2025-09-01-preview' = {
  parent: apim
  name: 'managed-identity-mcp'
  properties: {
    type: 'mcp'
    apiType: 'mcp'
    displayName: 'Managed Identity MCP'
    path: 'mcp/managed-identity'
    protocols: [
      'https'
    ]
...
resource product 'Microsoft.ApiManagement/service/products@2024-05-01' = {
  ...
  properties: {
    ...
    description: 'Test product for the managed-identity Responses API'
...
resource testSubscription 'Microsoft.ApiManagement/service/subscriptions@2024-05-01' = {
  ...
  properties: {
    displayName: 'Managed Identity Test'
    scope: product.id
    state: 'active'
    allowTracing: false
  }
...
output mcpEndpoint string = '${apim.properties.gatewayUrl}/mcp/managed-identity/mcp'
