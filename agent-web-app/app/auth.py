    ...
    async def validate(self, authorization: str | None) -> str:
        ...
        try:
            ...
            claims = jwt.decode(
                ...
                issuer=self._issuer,
                audience=[
                    self._settings.entra_api_client_id,
                    self._settings.entra_api_audience,
                ],
            )
        ...
        if self._settings.entra_scope_name not in scopes:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail="The required API scope is missing.",
            )

        if claims.get("azp") != self._settings.entra_spa_client_id:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
